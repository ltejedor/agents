...
def main():
    ...
    research_agent = ToolCallingAgent(
        ...
        name="research_agent",
        add_base_tools=True,
        description="Researches best practices for creating development documentation, especially for users new to a system",
    )

    user_research_agent = CodeAgent(
        model=model,
        tools=[],
        name="planning_agent",
        add_base_tools=True,
        additional_authorized_imports=["bs4"],
        description="Expert user researcher who knows how to create user-friendly experiences and does web research on what other developers have done with specific system to improve final documentation product",
    )

    documentation_writer = CodeAgent(
        model=model,
        tools=[create_file],
        name="documentation_writer",
        add_base_tools=True,
        additional_authorized_imports=["watchdog", "bs4", "os"],
        description="Writes the actual documentation and files for the system",
    )

    proofreader_agent = CodeAgent(
        model=model,
        tools=[],
        name="proofreader_agent",
        add_base_tools=True,
        additional_authorized_imports=["watchdog", "importlib", "os"],
        description="It's this developer's first day on the job! They read through the documentation and report back if it's good enough for them to get started.",
    )
    ...
    manager_agent = CodeAgent(
        ...
        model=model,
        add_base_tools=True,
        additional_authorized_imports=["time", "json", "pydoc", "watchdog", "os"],
        managed_agents=[research_agent, user_research_agent, documentation_writer, proofreader_agent],
        max_steps=100
    )
